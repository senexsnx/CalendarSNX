...
(function () {
    ...
    */
    ...
    function showCreateDialog() {
        ...
        var leftPanel = middle.add("panel", undefined, "Monats-Slots");
        ...
        function refreshLists(selectIndex) {
            ...
            for (var a = 0; a < MONTH_COUNT; a++) {
                var item = assignedList.add("item", monthNamesUi[a]);
                item.subItems[0].text = assigned[a] ? baseName(assigned[a]) : "- leer -";
            }
        ...
        function updateInfo() {
            ...
            info.text = filled + "/12 Monatsbilder gesetzt. " + pool.length + " Bilder im Pool.";
        }
        ...
        function updatePreview() {
            ...
            if (poolList.selection) {
                ...
                path = assigned[assignedList.selection.index];
                label = monthNamesUi[assignedList.selection.index] + ": " + (path ? baseName(path) : "leer");
            }
        ...
        function loadAll(paths) {
            ...
            for (var a = 0; a < MONTH_COUNT; a++) {
                assigned.push(a < paths.length ? paths[a] : null);
            }

            for (var j = MONTH_COUNT; j < paths.length; j++) {
                pool.push(paths[j]);
            }

            refreshLists(0);

            if (paths.length < MONTH_COUNT) {
                alert("Es wurden nur " + paths.length + " Bilder gefunden. Für 12 Monatsseiten brauchst du mindestens 12 Bilder.");
            }
        }
        ...
        upBtn.onClick = function () {
            ...
            var tmp = assigned[idx - 1];
            assigned[idx - 1] = assigned[idx];
            assigned[idx] = tmp;
        ...
        downBtn.onClick = function () {
            var idx = selectedAssignedIndex();
            if (idx < 0 || idx >= MONTH_COUNT - 1) return;
        ...
        toPoolBtn.onClick = function () {
            ...
            if (assigned[idx]) addUniquePath(pool, assigned[idx]);
            assigned[idx] = null;
            pool.sort(pathSort);
        ...
        swapBtn.onClick = function () {
            ...
            if (a < 0 || p < 0) {
                alert("Links einen Monat und rechts ein Poolbild auswählen.");
                return;
            }
        ...
        sortBtn.onClick = function () {
            ...
            all = uniqueExistingImagePaths(all);
            all.sort(pathSort);

            assigned = [];
            pool = [];

            for (s = 0; s < MONTH_COUNT; s++) {
                assigned.push(s < all.length ? all[s] : null);
            }

            for (s = MONTH_COUNT; s < all.length; s++) {
                pool.push(all[s]);
            }
    ...
    function buildCoverPage(doc, page, options, layers, palette, styles) {
        ...
        img.insertLabel(LABEL_MONTH_INDEX, "0");
        tryPlaceImage(img, options.assigned[0], "Deckblatt");
